...
get_balanced_dataset = function(data, w_gender=0){
  ...
  data_train = rbind(a_s, b_s, d_s)
  data_test = rbind(a_t, b_t, d_t)
  
  rm(a,b,c,d, a_s, b_s, d_s, a_t, b_t, d_t)
...
```{r}
tripdata_2020_r = read.csv(load_dataset("SL_dataset", "tripdata_2019_r.csv"))
tripdata_2020_r$age_groups = 0

#split_ages = c(16, 25, 35, 45, 55, 65, 75, 80)
split_ages = c(16, 30, 45, 65)

cat("min age:", min(tripdata_2020_r$age))
cat("\nmax age:", max(tripdata_2020_r$age))
cat("\nusers >60y:", nrow(tripdata_2020_r[tripdata_2020_r$age > 65,]))
cat("\nusers = 0y:", nrow(tripdata_2020_r[tripdata_2020_r$age == 0,]))
cat("\nusers >0 & <= 60:", nrow(tripdata_2020_r[tripdata_2020_r$age <= 65 & tripdata_2020_r$age > 0,]))



sub_data = tripdata_2020_r[tripdata_2020_r$age <= 65,]
...
data_train = as.data.frame(rbind(a_s, b_s, c_s))
data_test = as.data.frame(rbind(a_t, b_t, c_t))

labels_train = data_train$age_groups
labels_test = data_test$age_groups

data_train_a = subset(data_train, select = -c(age_groups))
data_test_a = subset(data_test, select = -c(age_groups))
...
data_train = data_train[1:500000,]
labels_train = labels_train[1:500000]
...
```{r}
table(tripdata_2020_r$month)

for (i in 1:12){
  print(nrow(tripdata_2020_r[tripdata_2020_r$month == i & tripdata_2020_r$age == 0,]))
}
```
...
data_test = subset(data_test, select = -c(gender, age_groups)) #gender? perchè lo togliamo?
...
tripdata_2020_r[tripdata_2020_r$age <= 80 & tripdata_2020_r$age > 0,]$age_groups = sub_data$age_groups
