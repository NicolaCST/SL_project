...
data_u85 = tripdata_2020_r[tripdata_2020_r$age <= 85 & tripdata_2020_r$age > 0,]
data_u85$age_groups = cut(data_u85$age, breaks=split_ages, include.lowest = TRUE, labels = FALSE)

data_o85 = tripdata_2020_r[tripdata_2020_r$age > 85 | tripdata_2020_r$age == 0,]

data_u85 = subset(data_u85, select = -c(age, birth.year, year, start.station.name, end.station.name))
data_o85 = subset(data_o85, select = -c(age, age_groups, birth.year, year, start.station.name, end.station.name))
...
c1 = density(data_u85[data_u85$age_groups == 1,]$month, adjust = 7)
c2 = density(data_u85[data_u85$age_groups == 2,]$month, adjust = 7)
c3 = density(data_u85[data_u85$age_groups == 3,]$month, adjust = 7)
...
```{r}
################# == 0 ################
dec_age = predict(rf_model, newdata = data_o85)

table(as.data.frame(dec_age))

data_o85$age_groups = dec_age

data_age = rbind(data_u85, data_o85)
```
