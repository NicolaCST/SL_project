...
```{r}
saveRDS(rf_model, "rf_best_ng_r_100p3.rda")
```
...
prob_1=predict(rf_model_one, data_train)

b = as.data.frame(prob_1)
colnames(b) = c("pred_b")
...
prob=predict(rf_model_two, data_train)
...
for (i in (1:length(final))){
  if(pred_1a[i] == pred_1b[i]){
    final[i] = pred_1a[i]
  }else{
    prob_pred_1 = df_prob_1[i,]
    prob_pred_2 = df_prob[i,]
    ...
    val1 = 0.55 * (as.numeric(w_class[pos1]) * m1)
    val2 = 0.45 * (as.numeric(w_class[pos2]) * m2)
