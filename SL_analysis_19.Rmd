...
In this section we will use a tool named Correlation matrix in order to quantify the strength of the linear relationship between two variables. We are basically trying to observe the level of change in one variable due to the change in the other one. We do this because strong relation between predictors may lead to a less performing model, we observe several predictors with a high correlation factor: -startday, stopday: those variables are correlated because it is unlikely for a user to start and end the trip in different days (unless the trip starts late at night but this eventuality covers a small percentage of the whole dataset) -starttime, stoptime: the correlation between these two features is expecially dominant between subscribers since they have a limited amuont of time to complete their trips before paying extra money
...
```{r}
#Pos Cor
corr_1<- ggplot(data_train, 
                aes(startday, stopday)) 
                + geom_jitter(color = "blue") + xlab("startday") + ylab("stopday")

corr_2<- ggplot(data_train, 
                aes(starttime, stoptime)) + 
                geom_jitter(color = "blue") + xlab("starttime") + ylab("stoptime")
...
tab_test_knn_age = readRDS('cm_knn.RData')
plot_stats(tab_test_knn, mode="test", cf = T)
...
```{r}
####################### LOAD KNN ################################
tab_test_knn_gen = readRDS('cm_knn_gender.RData')
...
```{r}
tab_group_20 = as.data.frame(table((data_2020$age_groups)))
tab_group_20$perc = round((tab_group_20$Freq / sum(tab_group_20$Freq))*100,2)
colnames(tab_group_20) = c("Group", "Count", "Perc")
tab_group_20
```
...
legend("topright", legend = start_tab_20$start_name,
       fill = viridis(nrow(start_tab_20)), cex = 0.8)
...
legend("topright", legend = end_tab_20$end_name,
       fill = viridis(nrow(end_tab_20)), cex = 0.8)
```
