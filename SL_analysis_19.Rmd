...
>>>>>>> 5ebc04403a33295fb422edc499ffc4bea341ea83
  ##extract testing set
  a_t = a[ran_test,]
  b_t = b[ran_test,]
  c_t = c[ran_test,]
...
data_u85 = tripdata_2019_r[tripdata_2019_r$age <= 80,]
data_u85$age_groups = cut(data_u85$age, breaks=split_ages, include.lowest = TRUE, labels = FALSE)

data_o85 = tripdata_2019_r[tripdata_2019_r$age > 80,]
data_o85$age_groups = 0

data_u85 = subset(data_u85, select = -c(age, birth.year))
data_o85 = subset(data_o85, select = -c(age, birth.year))
```
...
nrow(data_u85[data_u85$age_groups == 4,])
```
...
for (i in 1:nrow(data_u85)){
  dates[i] = paste(data_u85$year[i], data_u85$month[i], data_u85$startday[i], sep="-")
}
...
for (i in 1:nrow(data_o85)){
  dates[i] = paste(data_o85$year[i], data_o85$month[i], data_o85$startday[i], sep="-")
}
dates <- weekdays(as.Date(dates))
...
```{r}
X = subset(data_u85, select = -c(start.station.id,
                                 start.station.name,
                                 end.station.id,
                                 end.station.name,
                                 bikeid,
...
#our Y
age_groups = data_u85$age_groups
```
...
model = lm(age_groups~. -startday -stopday -tripduration, data=X)
summary(model)
```
...
```{r}
corr_data = subset(data_u85, select = -c(start.station.name, end.station.name, dayname, year))
...
```
```{r}
customer_dur$tripduration[16] > 10000
customer_dur$tripduration > 10000
a = customer_dur[customer_dur$tripduration > 4000,]
dim(a)
length(a)
```
...
##run knn function
pr = knn(data_train, data_test, cl=labels_train, k=1)
