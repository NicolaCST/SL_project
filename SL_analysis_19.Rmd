...
test_data = tripdata_2019_r[tripdata_2019_r$age>80,]
...
sub_data = subset(tripdata_2019_r, select = -c(year, birth.year, start.station.name, end.station.name))

control = rfeControl(functions = rfFuncs, method="cv", number=10)
results = rfe(sub_data[1:500000,1:15], sub_data[1:500000,16], rfeControl = control)
...
#our Y
age_groups = sub_data$age_groups
```
...
```{r}
model = lm(age_groups~tripduration + starttime + month + end.station.id + start.station.id, data=X)
summary(model)
```
...
```{r}
sub_data1 = subset(sub_data, select = -c(year, birth.year, start.station.name, end.station.name))
round(var(sub_data1),2)
res2<-rcorr(as.matrix(sub_data1))
