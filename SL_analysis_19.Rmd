...
data_train_one = as.data.frame(one[[1]])
data_test = setdiff(data_u85, data_train_one)


library(h2o)
rm(tripdata_2019_r, data_o85, one)
data_test = data_test[sample(1:nrow(data_test), 0.3*nrow(data_test)),]
rm(tripdata_2019_r, data_o85, one)


h2o.init()
data_train_one$age_groups = as.factor(data_train_one$age_groups)

train_df_h2o<-as.h2o(data_train_one)
test_df_h2o<-as.h2o(data_test)
...
perf_rf2_test <- h2o.performance(my_rf2, newdata = test_df_h2o)

h2o.confusionMatrix(my_rf)
h2o.confusionMatrix(my_rf2)
...
data_train_one = subset(data_train_one, select = -c(age_groups))

rf_model_one = randomForest(x = data_train_one,
                            y = as.factor(labels_train_one),
                            ntree = 150)
...
####### PREDICT #########
pred_1 = predict(rf_model_one, newdata = subset(data_test, select = -c(age_groups)))
