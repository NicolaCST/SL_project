...
get_balanced_dataset = function(data, balanced_gender=0, w_gender=0){
...
```{r}
table(data_u85$gender[data_u85$gender != 0], data_u85$age_groups[data_u85$gender != 0])
table(data_train$gender[data_o85$gender != 0], data_o85$age_groups[data_o85$gender != 0])

table(data_train$gender)
...
data_u85[c("tripduration", "starttime", "stoptime")] = lapply(data_u85[c("tripduration", "starttime", "stoptime")], nor)
```
...
```{r}
min(table(data_train$age_groups[data_train$gender == 2]))
table(data_train$age_groups[data_train$gender == 1])
...
pred_mda_train = mixture_model %>% predict(data_test)
tab_test_mda = table(pred_mda_train, data_test$age_groups)
...
```{r}
'''
  n_var  acc   kappa   acc_sd   kappa_sd
1	  4	  0.8194	0.2796	0.002305	0.010851	
2	  5	  0.8233	0.2823	0.001353	0.020734	
3	  6	  0.8279	0.3146	0.001405	0.008930	
4	  7	  0.8273	0.3151	0.001297	0.008708	
5	  8	  0.8450	0.3933	0.001821	0.008760	
6	  9	  0.8505	0.3988	0.001577	0.007542	*
7	 10	  0.8482	0.3793	0.001710	0.009814	
8	 11	  0.8492	0.3816	0.002060	0.009514	
9	 12	  0.8494	0.3793	0.001824	0.010752	
10 13	  0.8446	0.3402	0.001772	0.011093	
11 14	  0.8399	0.3077	0.002545	0.015396	
12 15	  0.8370	0.2802	0.001995	0.012672	
...
```{r}
full = get_balanced_dataset(data_u85, w_gender = 1, balanced_gender = 1)
data_train = as.data.frame(full[1])
data_test = as.data.frame(full[2])
...
full = get_balanced_dataset(data, w_gender = 1, balanced_gender = 1)
data_train = as.data.frame(full[1])
data_test = as.data.frame(full[2])
...
```
```{r}
barplot(table(data_train$age_groups))
