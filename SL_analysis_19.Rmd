...
In this section we will use a tool named Correlation matrix in order to quantify the strength of the linear relationship between two variables. We are basically trying to observe the level of change in one variable due to the change in the other one. We do this because strong relation between predictors may lead to a less performing model, we observe several predictors with a high correlation factor:

-startday, stopday: those variables are correlated because it is unlikely for a user to start and end the trip in different days (unless the trip starts late at night but this eventuality covers a small percentage of the whole dataset)

-starttime, stoptime: the correlation between these two features is expecially dominant between subscribers since they have a limited amuont of time to complete their trips before paying extra money
...
Here we show how these couples of feature are distribuited with respect to each other
...
```{r}
test = ensemble_predict(subset(data_train, select = -c(age_groups)))

# test result
tab = table(test$fin, data_train$age_groups)
plot_stats(tab, cf = T)
```
...
data_2020 = rbind(data_u85_20, new)
```

```{r}
a = ensemble_predict(subset(data_u85_20, select = -c(age_groups)))

# test result
tab = table(a$fin, data_u85_20$age_groups)
plot_stats(tab, cf = T)
```
...
c1 = density(data_2020[data_2020$age_groups == 1,]$month, adjust = 3.5)
c2 = density(data_2020[data_2020$age_groups == 2,]$month, adjust = 3)
c3 = density(data_2020[data_2020$age_groups == 3,]$month, adjust = 3)
c4 = density(data_2020[data_2020$age_groups == 4,]$month, adjust = 3)
