...
data_u80 = tripdata_2019_r[tripdata_2019_r$age <= 80,]
data_u80$age_groups = cut(data_u80$age, breaks=split_ages, include.lowest = TRUE, labels = FALSE)

data_o80 = tripdata_2019_r[tripdata_2019_r$age > 80,]
data_o80$age_groups = 0

data_u80 = subset(data_u80, select = -c(age, birth.year))
data_o80 = subset(data_o80, select = -c(age, birth.year))
```

```{r}
split_ages = c(16, 30, 45, 65, 85)
data_u80 = tripdata_2019_r[tripdata_2019_r$age <= 85,]
data_u80$age_groups = cut(data_u80$age, breaks=split_ages, include.lowest = TRUE, labels = FALSE)

data_o80 = tripdata_2019_r[tripdata_2019_r$age > 85,]
data_o80$age_groups = 0
...
cat("\nusers = 0y:", nrow(tripdata_2019_r[tripdata_2019_r$age == 0,]))
cat("\nusers >0 & <= 85:", nrow(tripdata_2019_r[tripdata_2019_r$age <= 85 & tripdata_2019_r$age > 0,]))
nrow(data_u80[data_u80$age_groups == 4,])
```
...
```{r}
dates <- vector(mode="character", length=nrow(data_u80))
for (i in 1:nrow(data_u80)){
  dates[i] = paste(data_u80$year[i], data_u80$month[i], data_u80$startday[i], sep="-")
}
dates <- weekdays(as.Date(dates))
data_u80$dayname = dates

dates <- vector(mode="character", length=nrow(data_o80))
for (i in 1:nrow(data_o80)){
  dates[i] = paste(data_o80$year[i], data_o80$month[i], data_o80$startday[i], sep="-")
}
dates <- weekdays(as.Date(dates))
data_o80$dayname = dates

data_u80 = subset(data_u80, select = -c(year))
data_o80 = subset(data_o80, select = -c(year))

tab = table(data_u80$dayname)
barplot(tab)
```
...
```{r}
X = subset(data_u80, select = -c(start.station.id,
                                 start.station.name,
                                 end.station.id,
                                 end.station.name,
                                 bikeid,
...
#our Y
age_groups = data_u80$age_groups
```
...
```{r}
model = lm(age_groups~. -startday -stopday, data=X)
summary(model)
```
...
```{r}
#this appear to be the best subset
model = lm(age_groups~. -startday -stopday -bikeid -tripduration, data=X)
summary(model)
```
...
```{r}
corr_data = subset(data_u80, select = -c(start.station.name, end.station.name, dayname, year))
...
tab = table(data_u80$gender, data_u80$usertype)
hist = barplot(tab, 
        beside = TRUE, 
        legend = FALSE,
...
```{r}
EDA_UT = subset(data_u80, select = c(tripduration, usertype))
...
data_train = subset(data_train, select = -c(gender, age_groups))
data_test = subset(data_test, select = -c(gender, age_groups))
...
un_data = subset(unknown, select = -c(#startday, 
                                    stopday,
                                    start.station.name,
                                    start.station.id, 
                                    start.station.latitude, 
...
data_train = subset(data_train, select = -c(age_groups, gender))
data_test = subset(data_test, select = -c(age_groups, gender)) #gender? perchè lo togliamo?
...
```{r}
data = subset(data_u80, select = c(age_groups,
                                  start.station.name,
                                  start.station.latitude, 
                                  start.station.longitude,
                                  end.station.name,
...
```{r}
map_start = subset(data_u80, select = c(start.station.name,
                                  start.station.latitude, 
                                  start.station.longitude))
...
map_end = subset(data_u80, select = c(end.station.name,
                                  end.station.latitude, 
                                  end.station.longitude))
...
```{r}
pred_dataset = subset(sub_data, select = -c(year, 
                                    birth.year, 
                                    #startday, 
                                    stopday,
                                    #end.station.id,  
...
sub_data$gender[sub_data$gender == 0] = y_gender
...
```{r}
pred_dataset_age = tripdata_2019_r[tripdata_2019_r$age >= 90,]

pred_dataset_age = subset(pred_dataset_age, select = -c(year, 
                                                        birth.year, 
                                                        #startday, 
                                                        stopday,
                                                        #end.station.id,  
