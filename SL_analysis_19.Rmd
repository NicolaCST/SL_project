...
In this section we will use a tool named Correlation matrix in order to quantify the strength of the linear relationship between two variables. We are basically trying to observe the level of change in one variable due to the change in the other one. We do this because strong relation between predictors may lead to a less performing model.
...
-   starttime, stoptime: the correlation between these two features is expecially dominant between subscribers since they have a limited amount of time to complete their trips before paying extra money
...
a$fin = as.numeric(final)
saveRDS(as.data.frame(a), file = "Pred_ready/predict_age19.Rdata")
...
```{r}
pred_age_2019 = readRDS("Pred_ready/predict_age19.Rdata")
...
|-----------|---------|---------|---------|---------|
| Precision | 70.45   | 70.94   | 77.71   | 88.11   |
| Recall    | 71.67   | 66.04   | 80.47   | 97.25   |
...
-   Group 2: even if most of the stations are located in the Cambridge district, there are two that are located in different places. We can find one in the financial district, and the other one in the Black Bay
...
Further hypotheses can be formulated taking into account the most popular routes for each age group, we indicate with a blue sign the starting stations and with an orange sign the ending ones.

#### Group 1

We can see that the first group (16-30 y.o) does most their trips in the upper-west side of the city, where the biggest colleges and universities are located, near the Fenway-Kenmore neighborhood, place of some other important educational institutes, and in Beacon Hill, one of the historical and monumental parts of Boston. This may be caused by the amount of students that uses this service to reach the university district. Moreover we can assume in the case of overlapping starting and ending points that there are users living in the nearby area

![](images/crop1_19.png)

### Group 2

Group 2 (30-45 y.o.) seems the one with the longer and more spread trips. While Cambridge is still one of the most active district, users from group 2 often visits other part of the city like the southern residential neighborhood and also the financial district. This may be caused mainly by the working routine of the users of this category.
...
People from group 4 (65-85 y.o.) has the most varied trips of all, moreover they have an unforseen number of trips starting from Cambridge. We can find very pecurial routes, like the one going from the lake to Cambridge, from Cambridge to Beacon Hill passing through Longfellow Bridge.
...
From 11/05 instead, the first reopenings were announced: from this moment up to September, the restrictive measures have been gradually removed.
