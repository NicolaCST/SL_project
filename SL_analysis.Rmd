...
start = unique(EDA_id_station$start.station.name)
end = unique(EDA_id_station$end.station.name)
setdiff(end,start)

start_tab = data.frame(matrix(NA, nrow = length(start), ncol = 4))
colnames(start_tab) <- c("1","2", "3", "4")


for (rowIdx in 1:(nrow(start_tab))) {
  for (colIdx in 1:(ncol(start_tab))) {
    start_tab[rowIdx, colIdx] = nrow(subset(EDA_id_station, age == colIdx & start.station.name == start[rowIdx]))
  }
}

start_tab$start_name = start
...
bar = barplot(as.matrix(subset(a, select = -c(start_name))), 
        beside = T,
        cex.names = 0.5,
        las=2,
        col = heat.colors(5))
