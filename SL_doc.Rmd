...
```
```{r}
temp = tripdata_2019_r[tripdata_2019_r$age < 80,] 
```
...
X = subset(temp, select = c(tripduration, starttime, month, start.station.id, end.station.id))
y = temp$age
...
```{r}
model = lm(y~tripduration + starttime + month + start.station.id + end.station.id, data=temp)
summary(model)
```
```{r}
...
```
```{r}
model = lm(y~tripduration + starttime + month, data=temp)
summary(model)
```
```{r}
summary(model)$coefficient
